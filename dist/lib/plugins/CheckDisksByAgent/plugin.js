...
module.exports = class CheckDisksByAgent extends BasePlugin {
	...
	exec(args, success, error, service){
        ...
        .then( (agent)=>
        {
		    ...
		    var url = agent.getUrl()+"/_plugin/checker/check/disks?"
		  
		    var fs = []
		   
		    if (args.fs){
		        for (let i=0; i<args.fs.length; i++)
		            fs.push(args.fs[i].path)
		        url += 'fs='+fs.join(',')
		    }
